import React from 'react'
import Section from './Section'
import brackets from '../assets/svg/Brackets'
import { roadmap } from './constants'
import Button from '../Button'
import { grid } from '../assets'
import { Gradient } from './design/Hero'
import { check2 } from '../assets'
import { loading1 } from '../assets'
const LastSection = () => {
    return (
    <Section>
      <div className="container relative z-2 mx-auto">
        <div className="w-full flex justify-center gap-3 scale-150">
          {brackets('left')}
          <span className="relative -top-[5px] text-base opacity-40">Ready to get started</span>
          {brackets('right')}
        </div>

        <div className="mt-2">
          <p className="text-4xl lg:text-center lg:text-5xl">What we're working on</p>
        </div>

        <div className="relative grid gap-6 md:grid-cols-2 md:gap-4 md:pb-[7rem]">
        {roadmap.map((item) => {
          const status = item.status === "done" ? "Done" : "In progress";

          return (
            <div
              className={`md:flex even:md:translate-y-[8rem] p-0.25 rounded-[2.5rem] ${
                item.colorful ? "bg-conic-gradient" : "bg-n-6"
              }`}
              key={item.id}
            >
              <div className="relative p-8 bg-n-8 rounded-[2.4375rem] overflow-hidden xl:p-15">
                <div className="absolute top-0 left-0 max-w-full">
                  <img
                    className="w-full"
                    src={grid}
                    width={550}
                    height={550}
                    alt="Grid"
                  />
                </div>
                <div className="relative z-1">
                  <div className="flex items-center justify-between max-w-[27rem] mb-8 md:mb-20">
                  {brackets('left')}
                    <span className="relative -top-[5px] text-base opacity-40">{item.date}</span>
                  {brackets('right')}

                    <div className="flex items-center px-4 py-1 bg-n-1 rounded text-n-8">
                      <img
                        className="mr-2.5"
                        src={item.status === "done" ? check2 : loading1}
                        width={16}
                        height={16}
                        alt={status}
                      />
                      <div className="tagline">{status}</div>
                    </div>
                  </div>

                  <div className="mb-10 -my-10 -mx-15">
                    <img
                      className="w-full"
                      src={item.imageUrl}
                      width={628}
                      height={426}
                      alt={item.title}
                    />
                  </div>
                  <h4 className="h4 mb-4">{item.title}</h4>
                  <p className="body-2 text-n-4">{item.text}</p>
                </div>
              </div>
            </div>
          );
        })}

        <Gradient />
      </div>

      <div className="flex justify-center mt-12 md:mt-15 xl:mt-20">
        <Button href="/roadmap">Our roadmap</Button>
      </div>
    </div>
  </Section>
);}
export default LastSection;
